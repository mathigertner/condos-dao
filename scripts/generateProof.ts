...
async function generateSemaphoreProof(
  ...
  try {
    ...
    const privateKeyArray = identityData.privateKey
      .split(",")
    ...
    if (memberIndex === -1) {
      throw new Error(
        "Identity commitment not found in local group. Add it with scripts/merkleTree.ts add."
      );
    }
    ...
    console.log(
      `🔍 Local proof verification: ${isValid ? "VALID" : "INVALID"}`
    );
...
async function main() {
  ...
  switch (command) {
    case "generate":
      ...
      if (!daoAddress) {
        console.log(
          "ℹ️  No daoAddress provided. Use 'local' to run in offline mode."
        );
        console.log(
          "Usage: npx ts-node scripts/generateProof.ts generate [identityPath] [daoAddress|local] [proposalId] [voteValue] [rpcUrl]"
        );
        return;
      }

      await generateSemaphoreProof(
        identityPath,
        daoAddress,
        proposalId,
        voteValue,
